...
load('~/Figure2/data/NHP_data_all_sessions.mat') 
dataU=data_session1; %you need to change the session # here. We have four sessions 
...
for s=1:length(datasets)   
    ...
    for r=1:nRepeat
       ...
       for i=1:numY0
        ...
        for n=minN:maxN
            %% 1. BL: No AL
            svrMdl1=fitrsvm(X(idsTrain(1,1:n),:),Y(idsTrain(1,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            svrMdl2=fitrsvm(X(idsTrain(2,1:n),:),Y(idsTrain(2,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            for i=1:nBoots
            svrMdl=fitrsvm(X(idsTrain(2,C(i,:)),:),Y(idsTrain(2,C(i,:))),'Standardize',true,'KernelFunction','rbf');
            ...
            svrMdl3=fitrsvm(X(idsTrain(3,1:n),:),Y(idsTrain(3,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            for k=1:n+1
                if sum(ismember(idsTrain(3,1:n),idsP{k}))==0
                    ...
                    for i=1:nBoots
                        ...
                        svrMdl=fitrsvm(X(idsTrain(3,C(i,:)),:),Y(idsTrain(3,C(i,:))),'Standardize',true,'KernelFunction','rbf');
            ...
            svrMdl4=fitrsvm(X(idsTrain(4,1:n),:),Y(idsTrain(4,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            for i=1:nBoots            
            svrMdl=fitrsvm(X(idsTrain(4,C(i,:)),:),Y(idsTrain(4,C(i,:))),'Standardize',true,'KernelFunction','rbf');
            Ys(idsUnlabeled,i)=predict(svrMdl,X(idsUnlabeled,:));
            end
...
%             end
            ...
            svrMdl5=fitrsvm(X(idsTrain(5,1:n),:),Y(idsTrain(5,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            for k=1:n+1
                if sum(ismember(idsTrain(5,1:n),idsP{k}))==0
                    Ys=repmat(Y(idsP{k}),1,nBoots);
                    for i=1:nBoots
                        svrMdl=fitrsvm(X(idsTrain(5,C(i,:)),:),Y(idsTrain(5,C(i,:))),'Standardize',true,'KernelFunction','rbf');
                        Ys(:,i)=predict(svrMdl,X(idsP{k},:));    
            ...
            %% 6. GS
            svrMdl6=fitrsvm(X(idsTrain(6,1:n),:),Y(idsTrain(6,1:n)),'Standardize',true,'KernelFunction','rbf');
...
%             end
            svrMdl7=fitrsvm(X(idsTrain(7,1:n),:),Y(idsTrain(7,1:n)),'Standardize',true,'KernelFunction','rbf');
...
%             end
            ...
            svrMdl8=fitrsvm(X(idsTrain(8,1:n),:),Y(idsTrain(8,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            %% 9. EQBC
            svrMdl9=fitrsvm(X(idsTrain(9,1:n),:),Y(idsTrain(9,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            for i=1:nBoots
             ...
             svrMdl=fitrsvm(X(idsTrain(9,C(i,:)),:),Y(idsTrain(9,C(i,:))),'Standardize',true,'KernelFunction','rbf');
...
%             end
            svrMdl10=fitrsvm(X(idsTrain(10,1:n),:),Y(idsTrain(10,1:n)),'Standardize',true,'KernelFunction','rbf');
            ...
            Ys=repmat(Y,1,nBoots);
            for i=1:nBoots
             svrMdl=fitrsvm(X(idsTrain(10,C(i,:)),:),Y(idsTrain(10,C(i,:))),'Standardize',true,'KernelFunction','rbf');
...
save('~/Figure2/results/NHP_SVR_RBF_S1','MSEs','CCs') %please change this directory to avoid overwritting the shared result!
