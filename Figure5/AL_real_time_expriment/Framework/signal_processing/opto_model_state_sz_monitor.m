classdef opto_model_state_sz_monitor < handle
    ...
    methods
        function obj = opto_model_state_sz_monitor(TD_FS, channels, metric_type)
        ...
        function m = get_metric(obj,window_start_time, window_end_time, plot_flag)
            ...
            %             end
            start_index         = obj.DATA_BUFFER.lst - obj.DATA_BUFFER.fst - int64(round((obj.data_update_time - window_start_time) * obj.data_sampling_frequency));
            if (start_index <0) || isempty(start_index)
                start_index = 1;
            end
            end_index           = obj.DATA_BUFFER.lst - obj.DATA_BUFFER.fst - int64(round((obj.data_update_time - window_end_time) * obj.data_sampling_frequency));
            temp_seg = obj.DATA_BUFFER.raw(obj.DATA_BUFFER.fst:obj.DATA_BUFFER.lst,:);
            ...
            IIS_detected = BO_IIS_Detect_Power_Rolston(objective_data(:,Cheff),7.5,0.5,0.1,2000,0);
            n_spikes = size(IIS_detected.time_Sp_final{1},1);
            fprintf('%s total interictal spikes\n',num2str(size(IIS_detected.time_Sp_final{1},1)))
            spike_times = IIS_detected.time_Sp_final{1};
            m = n_spikes;
%             t_spike_detect = zeros(size(t_plot));
%             counter = 1; t_temp = t_plot - min(t_plot);
%             for k = 1:length(t_spike_detect)-1
%                 if (t_temp(k) >= spike_times(counter,1))
%                     t_spike_detect(k:k+1) = 1;
%                     counter = counter + 1;
%                     if counter > size(spike_times,1)
%                         break
%                     end
%                 end
%             end
            ...
            obj.METRIC_BUFFER.append(n_spikes);
        end
