...
function out = BO_IIS_Detect_Power_Rolston(data,S_thresh,W,MW,Fs,plot_flag)
...
[b,a] = butter(4,[10,100]/(Fs/2),'bandpass');
...
for C=1:1:size(V_filtered,1)
    ...
    for i=1:1:length(ind_detect)
        ...
        time_Sp_final(i,1:2) = [(1/Fs)*(ind_up(ind_detect(i)))-W/2, (1/Fs)*(ind_down(ind_detect(i)))+W/2];
        indraw_Sp_final(i,1:2) = [((1/Fs)*(ind_up(ind_detect(i)))-W/2)*2000, ((1/Fs)*(ind_down(ind_detect(i)))+W/2)*2000];
    end
